...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (prevVersion.current === dataVersion && prevVersion.current !== null) return;
      ...
      try {
        const [statsRes, voituresRes, ventesRes] = await Promise.all([
          ...
          supabase.from("ventes")
            .select("prix_vente_fcfa, date_vente, status, montant_recu_fcfa")
            .eq("status", "active")
            .gte("date_vente", format(subMonths(new Date(), 6), "yyyy-MM-dd")),
        ]);
        ...
        if (ventesRes.data) {
          ...
          setStats((prev) =>
            prev
              ? {
                  ...prev,
                  total_encaisse_fcfa: totalEncaisse,
                  total_restant_fcfa: Math.max(0, totalVentes - totalEncaisse),
                }
              : prev
          );
        }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/voitures" className="text-primary font-medium hover:underline">Voir toutes →</Link>
        </div>
        {recentVoitures.length === 0 ? (
          <Card><CardContent className="p-8 text-center"><p className="text-muted-foreground mb-4">Aucune voiture enregistrée</p><Link href="/voitures/ajouter" className="text-primary font-medium hover:underline">+ Ajouter votre première voiture</Link></CardContent></Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recentVoitures.map((v) => <VoitureCard key={v.id} voiture={v} />)}
          </div>
        )}
      </div>
    </div>
